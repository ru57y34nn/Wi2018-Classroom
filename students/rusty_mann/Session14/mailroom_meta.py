...
def save_donordb(db):
    ...
    try:
        with open('mailroom.json', 'w') as db_handle:
            db_handle.write(db.donor_db.donor_db.to_json())
    except IOError:
...
def add_donation(db, donor, contribution):
    ...
    if donor in db.donor_db.keys():
        db.donor_db[donor].append(float(contribution))
    else:
        db.donor_db[donor] = [float(contribution)]
