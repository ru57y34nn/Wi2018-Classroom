...
def make_letter_files():
    ...
    for donor in db.donor_db.keys():
        ...
        letter_dict["amt"] = db.donor_db[donor][-1]
...
def send_donor_email(db=donor_db, name=False, amount=False):
...
def make_report():
    ...
    rows = []
    for donor, values in db.donor_db.items():
        total = sum(values)
        num = len(values)
        avg = total / num
